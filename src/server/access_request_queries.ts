...
import { dataset, tags, datasetTags, access_request, user } from "./db/schema";
...
async function hasExistingPendingRequest(userId: string, datasetId: number) {
...
export async function checkPendingRequest(datasetId: number) {
  const session = await auth.api.getSession({
    headers: await headers(),
  });
  const user_id = session?.user.id;
...
export async function hasApprovedAccess(datasetId: number) {
  const session = await auth.api.getSession({
    headers: await headers(),
  });
  const user_id = session?.user.id;
